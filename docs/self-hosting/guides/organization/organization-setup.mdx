...
```bash
NEXT_PUBLIC_WEBAPP_URL=http://app.cal.local:3555
NEXT_PUBLIC_WEBSITE_URL=http://app.cal.local:3555
NEXTAUTH_URL=http://app.cal.local:3555
...
2. Visit `http://app.cal.local:3555/settings/organizations/new`
...
<Note>
After creating the organization, you'll be moved inside it and all existing Cal.com links will redirect to the organization URL (e.g., `yourdomain.cal.local:3555`).
</Note>
...
```
http://myorg.cal.local:3555/{CAL_LINK}
```
