...
describe("ApiAuthStrategy", () => {
  ...
  beforeAll(async () => {
    ...
    const data = {
      logo: "logo-url",
      name: "name",
      redirectUris: ["http://localhost:3555"],
      permissions: 32,
    };
  ...
  describe("authenticate with strategy", () => {
      ...
      const context: ExecutionContext = {
        switchToHttp: () => ({
          getRequest: () => ({
            ...
            get: (key: string) =>
              ({ Authorization: `Bearer cal_test_badkey1234`, origin: "http://localhost:3555" }[key]),
          }),
        }),
      } as ExecutionContext;
      ...
      const context: ExecutionContext = {
        switchToHttp: () => ({
          getRequest: () => ({
            ...
            get: (key: string) => ({ origin: "http://localhost:3555" }[key]),
          }),
        }),
      } as ExecutionContext;
      const request = context.switchToHttp().getRequest();
    ...
    it("should throw 401 if request does not contain Bearer token nor OAuth client credentials", async () => {
      const context: ExecutionContext = {
        switchToHttp: () => ({
          getRequest: () => ({
            get: (key: string) => ({ Authorization: ``, origin: "http://localhost:3555" }[key]),
          }),
        }),
      } as ExecutionContext;
      const request = context.switchToHttp().getRequest();
